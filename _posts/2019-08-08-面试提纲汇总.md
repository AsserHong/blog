...
* 事件循环
> + 概念：
		+ js一直在执行一个类似于while true的循环。事件任务分为宏任务和微任务。执行顺序为宏任务 -> 微任务 -> 宏任务 -> 微任务 ...
> + 宏任务包括：
		+ IO操作（ajax、点击事件等）、setTimeout、setInterval、setImmediate、requestAnimation、postMessage(在vue2.0的$nextTick中，就是优先使用setImmediate和MessageChannel的postMessage,最后才考虑使用setTimeout)
> + 微任务包括：
